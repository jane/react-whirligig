...
export const includes = (val: any, arr: any[]): bool =>
...
export const normalizeIndex = (idx: number, len: number, wrap: bool = false) =>
  wrap ? wrapAroundValue(idx, len) : hardBoundedValue(idx, len - 1)

export const values = Object.values || ((obj) =>
  Object.keys(obj).map((key) => obj[key]))

export const minMap = (...vals: number[]) =>
  (val: number): number =>
    Math.min(...vals, val)

export const maxMap = (...vals: number[]) =>
  (val: number): number =>
    Math.max(...vals, val)
...
export const onScroll = (
  ...
  onWindowScroll((e: SyntheticEvent<*>) => (target === window || target === e.target) && cb(e))
...
export const onSwipe = (cb: (string) => void) => (target: SyntheticTouchEvent<*>): void => {
...
export const trackOngoingMouseInteraction = (el: Element): () => bool => {
...
export const hasOngoingInteraction = (el: Element): () => bool => {
...
export const isWhollyInView = (parent: Element) =>
  (child: Element | typeof fakeChild = fakeChild): bool => {
